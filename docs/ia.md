...
All charts in [topic-map.md](./topic-map.md) (34 univariate charts across 8 topics) load via `/api/nomis` + `loadCensusSeries`, respect `?geography=`, plot count values with percent alongside in tooltips (`20100` + `20301`), use browser cache, and render with Recharts (`pie` / `bar` / `horizontal-bar`) plus shared Loading / Error / Stale states.
